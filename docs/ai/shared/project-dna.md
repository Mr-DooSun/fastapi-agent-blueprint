...
### Admin audit logging (#196 Phase 1)

- **Persistence**: `src/_core/infrastructure/admin/audit/` (`AdminAuditLog` model + `0007` migration). Append-only — Phase 1 ships `AdminAuditLogRepository.insert` only; the `/admin/audit-log` query UI and `@audit_action` decorator generalization ship in Phase 2.
...
- **Schema layout**: `action` is the verb (`LOGIN`, `ACCOUNT_DELETE`, …), `result` is `SUCCESS`/`FAILURE` — orthogonal, no `LOGIN_SUCCESS` compound. `record_id` is a varchar (UUID/string-id ready). `admin_username` is denormalized so log entries survive account deletion (FK is `ON DELETE SET NULL`). Composite indexes pair the common filter columns with `created_at DESC`.
