...
- [ ] **Language Policy** (AGENTS.md § Language Policy): Run `python tools/check_language_policy.py` and confirm zero violations across Tier 1 paths. Bilingual escape tokens (`[자명]`/`[긴급]`/`[탐색]`) are the sole exception, scoped per-file in the checker. Flag any new non-English prose insertion as a sync-required drift candidate.
