...
- [ ] [When applicable][MEDIUM] No working webhook URL is committed to env templates, compose files, or operations docs
  ...
  - **gitleaks backstops Slack only — not Discord.** `.pre-commit-config.yaml` pins `gitleaks v8.30.1` with no `.gitleaks.toml` override, so the shipped default ruleset applies, and CI runs it via `pre-commit run --all-files`. Its `slack-webhook-url` rule matches the whole URL with no variable-name anchor, so a real Slack webhook is blocked under *any* variable name — that is what makes the grep above a reviewer aid rather than the only control. There is **no Discord webhook rule**: `discord-api-token`, `discord-client-id` and `discord-client-secret` are `GenerateSemiGenericRegex` shapes that `discord.com/api/webhooks/<id>/<token>` cannot match. So for Discord the grep above is the *only* control, and a Discord webhook committed under a variable name it does not cover has **zero** coverage. Do not assume the hook catches what the grep misses. A custom rule closing the Discord gap is tracked in #320
...
- [ ] [When applicable][MEDIUM] Notification provider configuration is validated as a complete group at startup
  ...
  - Not a finding: the **designed opt-outs**. An unset `NOTIFICATION_PROVIDER` is a fallback, not a misconfiguration — the notification Selectors resolve to `NoopNotificationClient` when `settings.notification_webhook_url` is `None` (ADR 042), which is what keeps the zero-config quickstart booting. Likewise an unset `NOTIFICATION_WARNING_THRESHOLD` makes `notification_router()` resolve to `providers.Object(None)`, which is the documented #17 single-target path and not a disabled security control
  - Not a finding: the three notification *client* Selectors (`notification_client`, `notification_critical_client`, `notification_warning_client`) share **one** `_noop_notification_client` Singleton on their `disabled` branch, so the disabled path emits `notification_client_disabled` exactly once regardless of tier count. A reviewer expecting one Noop provider per Selector should not read the sharing as a wiring defect. The fourth Selector, `notification_router`, resolves to `providers.Object(None)` when disabled and never touches the Noop client — it is pinned by `test_noop_notification_client_disabled_warning_emitted_once`
...
- [ ] [When applicable][MEDIUM] Enabling channel routing does not widen the payload class, and the second channel gets the same treatment as the first
  ...
  - Not a finding: opening a warning band does **not** admit a new data class. The only un-redacted **HTTP** send site (`generic_exception_handler`) hard-codes `status_code=500`, so it is structurally unreachable below the severity threshold. The worker path also sends `str(exception)` verbatim for a non-`BaseCustomException`, but `_synthetic_status_code` scores those 500 as well, so that path cannot drop into the warning band either; every 4xx-capable site sends `str(BaseCustomException)`, which omits `details`. Assess routing as a **volume and audience** change, not an exposure one
  - Verify the warning channel is in scope for the same access and retention review as the critical channel — it receives production data on the same terms
  - Reason: the tempting review finding here ("routing leaks 4xx bodies") is wrong and has been asserted twice. The real risks are a mis-scoped second channel and a key/route divergence, neither of which a payload-width grep would catch
