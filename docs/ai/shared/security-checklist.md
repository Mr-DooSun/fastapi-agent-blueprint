...
- [ ] [When applicable][MEDIUM] Notification provider configuration is validated as a complete group at startup
  ...
  - Not a finding: all four notification Selectors (`notification_client`, `notification_critical_client`, `notification_warning_client`, `notification_router`) share **one** `_noop_notification_client` Singleton, so the disabled path emits `notification_client_disabled` exactly once regardless of tier count. A reviewer expecting one Noop provider per Selector should not read the sharing as a wiring defect — it is pinned by `test_noop_notification_client_disabled_warning_emitted_once`
...
- [ ] [When applicable][MEDIUM] Enabling channel routing does not widen the payload class, and the second channel gets the same treatment as the first
  ...
  - Not a finding: opening a warning band does **not** admit a new data class. The only un-redacted send site (`generic_exception_handler`) hard-codes `status_code=500`, so it is structurally unreachable below the severity threshold; every 4xx-capable site sends `str(BaseCustomException)`, which omits `details`. Assess routing as a **volume and audience** change, not an exposure one
