...
| **Drop** | Asset is removed because it is duplicated, dead, or superseded. | `tools/check_g_closure.py` (retired by ADR 047 PR B-F, #159 — see Update Log) |
...
- **Current role**: 467-line OWASP-aligned security review reference (13 categories; §13 Error Notification Security added by #305). Surface for `/security-review`.
...
Counting note: `Tier 0=14` (tool entry points and top-level governance files), `Tier 1=22` (18 Keep + 3 Overlay + 1 Drop — the 18 Keep are the 15 `docs/ai/shared/*.md` reference/design docs [incl. `admin-design-system.md`, `governor-paths.md`, and the 3 design living docs] + the frozen `governor-review-log/` directory + the `.agents/shared/governor/` package + `tools/check_governor_footer.py`; the 3 Overlay are `planning-checklists.md` / `drift-checklist.md` / `onboarding-role-tracks.md`; the 1 Drop is the historical `tools/check_g_closure.py`. No `docs/history/` ADR is a Tier 1 row — the matrix rows exactly one ADR, `045`, in Tier 0), `Tier 2=15` (skill rows; each row covers all 3 wrapper layers), `Tier 3=31` (#281 / ADR 054 added `.claude/hooks/pre-tool-stage-block.sh` + `.claude/hooks/pre_tool_stage_block.py` and rowed the previously-omitted `.claude/hooks/session-start-context.sh`; #268 added `.claude/hooks/stage-gate.sh` + `.claude/hooks/post_tool_stage_gate.py`; #269 folded Codex stage-gate into the existing Stop hook; Antigravity adds 8 Python hook shims; Phase 4 #123 = 18, Phase 3 = 16, Phase 2 = 13, Phase 1 = 10; Phase 5 #124 converted 6 of these to thin shims without changing the count), `Tier 4=7` — sum 89. The 88 figure above excludes `.claude/settings.local.json` from the matrix-row count because it is `.gitignore`d. The bucket-share percentages use 88 as the denominator (all 88 matrix rows, including the retired Drop, whose ~1% share is real; the "87 active" figure below excludes only that retired Drop row). (Tier 1 has 22 quick-table rows — 18 Keep, 3 Overlay, 1 Drop; a prior sync labelled this tier 21 and recorded base Keep as 54, undercounting the total by one Keep row — corrected here.)

This distribution matches the "Mostly Local with Philosophy Overlay" model declared in [ADR 045 §D4](../../history/045-hybrid-harness-target-architecture.md). The `Replace` column is empty (0): no asset's content is being rewritten. The `Drop` column has exactly one historical entry — `tools/check_g_closure.py`, a file removed by ADR 047 PR B-F and retained as a `Drop` row so the retirement stays traceable. (During the first triage the only `Drop` candidate then identified — a sh-wrapper `.py` pair — was overturned by self-verification as an active component; the `check_g_closure.py` Drop was recorded later, once that file was genuinely removed.)

If a future `Replace` candidate emerges, the threshold is: Keep+Overlay would otherwise force the asset into structural inconsistency with the Default Flow. None of the current 87 active assets meet that (the 88th matrix row is the retired `tools/check_g_closure.py` Drop, which is not a `Replace` candidate).
...
- [ ] Every asset on the filesystem appears in this matrix exactly once. Verify via:
  ```bash
  # Tier 0 (the only constitutional ADR row is docs/history/045-*.md; every other
  #  docs/history/NNN-*.md ADR is indexed by docs/history/README.md, not rowed here)
  ...
  ls -d .agents/shared/governor docs/history/archive/governor-review-log
...
- 2026-05-02 — ADR 047 PR B-F (#159): `tools/check_g_closure.py` retired (Drop); `tools/check_governor_footer.py` + `.github/workflows/governor-footer-lint.yml` added; `docs/history/047-governor-review-provenance-consolidation.md` added to Tier 1; `.agents/shared/governor/sync_cosmetic.py` added to Phase 5 package. `governor-review-log/` relocated to `docs/history/archive/governor-review-log/` (PR #161). Bucket-share ~79% Keep / ~20% Overlay / ~1% Drop. (Errata 2026-07-20 / #297: the "added to Tier 1" phrase above described a Tier 1 counting-note **text** insertion only — ADR 047 was never given an actual Tier 1 quick-table row; the matrix rows exactly one ADR, `045`, in Tier 0. See the 2026-07-20 #297 entry below.)
...
- 2026-07-20 — #297: corrected the Tier 1 counting-note breakdown, which wrongly listed `docs/history/047-*.md` as a Tier 1 row — 047 has no matrix row; the matrix rows exactly one ADR, `045`, in Tier 0. Companion `drift-checklist.md` §1C reconciliation with matrix practice: added `.github/pull_request_template.md` to the Tier 0 enumeration, added the non-`docs/ai/shared/` governor assets (`.agents/shared/governor/`, `tools/check_governor_footer.py`, `docs/history/archive/governor-review-log/`) to Tier 1, and replaced the unpassable "every ADR" clause with an explicit ADR inclusion rule delegating ADR completeness to `docs/history/README.md`. Drift filed from the PR #296 cross-review (round 3, Deferred-with-rationale). No asset count change (Total 88 → 88).
- 2026-07-20 — reconciled the §Bucket Distribution Summary narrative with its own table. Post-ADR-047 the `Drop` column holds one row (`tools/check_g_closure.py`, retained as a historical Drop after #159), but the narrative below the counting note still claimed "the `Replace` and `Drop` columns are both empty" and counted "88 active assets". Reworded to `Replace` empty (0) / `Drop` = 1 historical entry, and corrected the active-asset count to 87 (+1 retired Drop = 88). Pre-existing drift surfaced by the #297 cross-review (Codex round 1) as out-of-scope of that PR's §1C/matrix enumeration reconciliation. The PR's own independent cross-review then caught two same-drift-class misses, fixed in the same pass: (a) the top-of-doc Bucket Definitions legend (line 20) still showed the Drop Example as "(none in this initial inventory)" — updated to cite `tools/check_g_closure.py`; (b) the new "87 active" phrasing overloaded the word "active" against the counting note's "active-share count" (=88) — the counting note now reads "matrix-row count" with an explicit 88-rows-incl-retired-Drop vs 87-active note. No asset/count/bucket change (Total 88 → 88).
