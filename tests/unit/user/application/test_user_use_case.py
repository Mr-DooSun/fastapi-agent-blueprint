...
class MockUserService:
    ...
    async def create_data(self, entity: CreateUserDTO) -> UserDTO:
        dto = make_user_dto(id=self._next_id, **entity.model_dump())
        self._store[self._next_id] = dto
        self._next_id += 1
        return dto

    async def create_datas(self, entities: list[CreateUserDTO]) -> list[UserDTO]:
...
async def test_create_user(user_use_case):
    ...
    result = await user_use_case.create_data(entity=dto)
...
async def test_get_datas_returns_pagination(user_use_case):
    for i in range(3):
        await user_use_case.create_data(
            entity=make_create_user_dto(username=f"user{i}")
        )
