...
> Last synced: 2026-09-04 via the **`/fix-bug` skill hardening** — **reviewed, structural patterns unchanged**: documentation only, `src/` diff is zero, no contract moved. One rule this file's subject matter *does* touch was corrected in the skill rather than here: the old `/fix-bug` instruction to "fix at the lowest possible layer (prefer domain > infrastructure)" contradicts [AGENTS.md](../../AGENTS.md#responsibility-matrix) § Responsibility Matrix — "each concern has exactly one home" — because provider SDK calls, SDK exception translation (`error_mapper.py`) and DI wiring are **infrastructure-owned**, and pushing such a fix into a domain service breaks both the dependency direction and § Error Translation. The skill now says "the earliest boundary that **owns** the broken invariant". That wording had been in the skill since it was written and was carried forward unexamined for three rewrites; a cross-tool review caught it. Prior: 2026-08-13 via #401 (PR #402) — **reviewed, structural patterns unchanged**: `tests/` only, no `src/` change, no contract moved. The testing convention it established (load a harness hook by path, never by a basename that exists in more than one harness copy) is canonical in [`test-patterns.md`](../../docs/ai/shared/test-patterns.md) § Harness Hook Tests rather than duplicated here. Prior: 2026-08-13 via #394–#399 (typing `tests/`) — **one contract did change.** `BaseRepositoryProtocol.insert_datas`, `BaseRepository.insert_datas` and `BaseDynamoRepository.batch_put_items` now take `Sequence[BaseModel]`; the vector store already did. `list` is invariant, so the old signatures forced `BaseService.create_datas` to launder its argument through `cast(list[BaseModel], entities)` — widening deleted that cast rather than moving it. The canonical rule and its two limits are in [`project-dna.md`](../../docs/ai/shared/project-dna.md) § Two signature conventions, along with the consumer-local `Protocol` pattern now used five times (`_AuditLogSink`, `_SupportsEventHandler`, `_SupportsProps`, `_AdminAuthOperations`, `_UsageSummaryService`). Data flow, object roles and the Admin Page Config contract are otherwise untouched. Prior: 2026-08-12 via #380 / #381 / #375 / #387 — **reviewed, structural patterns unchanged.** Data flow, object roles, generic signatures and the Admin Page Config contract are all untouched: #380 is presentation-only (two-column composition, a status list replacing an AG Grid on one panel) and #381 / #375 / #387 are the type-check gate, which adds no runtime pattern. Three durable constraints from that work live elsewhere by design — the multi-column composition rule and the `c.data_grid` scope warning in [`admin-design-system.md`](../../docs/ai/shared/admin-design-system.md), and the pyright scope contract in [`project-dna.md`](../../docs/ai/shared/project-dna.md) § CI Type Check. One typing fact does touch this file's subject matter and is recorded here rather than duplicated: `BaseRepository`'s aggregate read primitives are now checked by CI, because `_core/infrastructure/persistence/rdb` entered the pyright gate in #383 — a wrong `_stable_order()` return type was found and fixed there. Prior: 2026-08-12 via #368 (PRs #369, #370, #371, #373) — `count_datas_by_day` added to `BaseRepository` + `BaseRepositoryProtocol` + `BaseService` (the pairing rule is recorded below), returning the new `DailyCount` VO. Data flow, object roles and generic signatures are otherwise unchanged. Prior: 2026-08-11 via #365/PR #366 + PR #367 (admin neutral-mono restyle — **reviewed, structural patterns unchanged**. Data flow, object roles, generic signatures, and the Admin Page Config contract below are all untouched: the change is presentation-only, confined to `theme.py` / `layout.py` / `components/charts.py` token values plus the removal of the `/admin-static` mount. One durable constraint worth knowing when editing the admin shell, canonical in [`admin-design-system.md`](../../docs/ai/shared/admin-design-system.md): the Quasar `--q-*` brand group must be emitted under `body` with `!important`, because NiceGUI writes that palette as an inline body style that outranks any `:root` rule — a `:root` declaration is silently inert, which is what shipped from #193 until #365). Prior: 2026-08-01 via #286/PR #313 + #315/PR #319 (Error Notification — severity channel routing: three client Selectors sharing one Noop Singleton plus a router Selector resolving to None, the `_effective_min_threshold()` floor, the tier-scoped cooldown key, and the band-test invariant. Data-flow / object-role / generic-signature patterns unchanged). Prior: 2026-07-28 via #310 (Error Notification — worker task-failure dispatch + middleware ordering contract added below; the server-only scope from PR #311 is superseded. Data-flow / object-role / generic-signature patterns unchanged). Prior: 2026-07-27 via #307/PR #311 (Error Notification runbook — added the server-only dispatch scope to the optional-infra section below; data-flow / object-role / generic-signature patterns unchanged). Prior: 2026-07-23 via #17/PR #304 (Error Notification webhook infra — new optional-infra section added below; existing data-flow / object-role / generic-signature patterns unchanged). Prior: 2026-06-01 via #218 (admin-identity realm separation reviewed; the new `admin_identity` domain reuses the existing data flow / object roles / generic-signature patterns — BaseService/BaseRepository generics and conversion patterns are unchanged. New realm invariants live in project-dna §17.)
...
- `BaseRepositoryProtocol(Protocol, Generic[ReturnDTO])` / `BaseRepository(Generic[ReturnDTO])` — 1 TypeVar plus read primitives for Service-owned validation. Write inputs that are only iterated take `Sequence[BaseModel]`, never `list[BaseModel]`: `list` is invariant, so the narrower spelling forces every caller holding `list[MyDTO]` into a cast. An implementation or test double with a *narrower* parameter than the protocol does not satisfy it — that is how #395 found two doubles that had drifted from the interfaces they impersonate
