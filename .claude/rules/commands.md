...
> Last synced: 2026-09-04 via the **`/fix-bug` skill hardening** — **reviewed, no command surface changed**: no new CLI command, env var, or target. One command moved *into* a workflow that had never named it — `/fix-bug` Phase 4 now runs `uv run pyright`, which this file has documented as the single blocking type check since #375 but which the bug-fix procedure never mentioned, so a fix could pass that skill's verify step and still fail CI. Prior: 2026-08-13 via #401 (PR #402) — **reviewed, no command surface changed**: the work is confined to how `tests/unit/agents_shared/` loads harness hook modules. `uv run pyright` still covers the whole repository (945 files, 0 errors). Prior: 2026-08-13 via #394–#399 — `uv run pyright` now covers the **whole repository** (944 files, 0 errors) including `tests/`; nothing about how you invoke it changed. Prior: 2026-08-12 via #375 / #381 / #387 (PRs #382–#386, #388, #390) — the Lint / Format section now names `uv run pyright` as the single type check and no longer claims `pre-commit run --all-files` runs mypy; that hook is gone. Two command-level consequences worth knowing: `make scheduler` **had never actually run the scheduler** before #375 (the coroutine was created and dropped, so the process exited 0 having scheduled nothing), and `make quickstart` runs `uv sync --extra admin`, which **uninstalls every other extra** — aws, otel, pydantic-ai, sqs, rabbitmq. The `dev` group survives (uv syncs default groups regardless of `--extra`), so the trap is not missing test tooling: inside a dev checkout `pytest tests/` then **aborts during collection** because two modules import `aioboto3`, and `uv run pyright` reports 47 unresolved imports. The target now detects this and prints a restore hint; corrected here after the first wording said "removes the dev extras", which measurement disproved. Prior: 2026-08-12 via #374 (the `make test-pg` clean-database caveat is gone — the fixture now loads every model, so a pre-existing schema no longer breaks it) and #368 (no new commands). Prior: 2026-08-11 via #365/PR #366 (Admin Dashboard — the UI theming block now names the neutral-mono theme instead of the Toss-style one, and records that the `--q-*` brand group must be emitted under `body` with `!important`, because NiceGUI writes that palette as an inline body style and a `:root` declaration is inert — the pre-#365 state, in which the whole brand half of the palette never applied). Prior: 2026-08-01 via #286/PR #313 + #315/PR #319 (Error Notification — severity channel routing section and example added; `:159` corrected, it had contradicted the Coverage bullet in the same block since #310). Prior: 2026-07-28 via #310 (Error Notification — dispatch extended to Taskiq worker task failures; the server-only scope note from PR #311 is superseded). Prior: 2026-07-27 via #307/PR #311 (Error Notification section — runbook pointer, server-only dispatch scope, first-dispatch disabled warning). Prior: 2026-07-23 via #17/PR #304 (added the Error Notification section — `NOTIFICATION_*` env vars for Slack/Discord webhook alerts fired from the exception handlers). Prior: 2026-07-20 via ADR 056 (added `tools/check_migration_safety.py` — advisory unsafe-DDL scan for zero-downtime migrations — to Architecture Verification + DB Migrations). Prior: 2026-07-20 via #293 (added `make perf-test` — Locust performance-test harness — to the Test section).
...
# NOTE: this runs `uv sync --extra admin`, so every *other* extra is uninstalled
# (aws, otel, pydantic-ai, sqs, rabbitmq). The `dev` group survives — uv syncs
# default groups regardless of `--extra` — so the damage is not to test tooling
# but to what it can import: `pytest tests/` aborts during collection (two modules
# import `aioboto3`) and `uv run pyright` reports 47 unresolved imports. The target
# prints a restore hint when it detects a dev environment; re-run `make setup`.
make quickstart
...
# UI theming (#193, restyled in #365): single neutral-mono theme — a desaturated
# Tailwind zinc ramp with one blue accent. Rebrand by editing the token dicts in
# src/_core/infrastructure/admin/theme.py (no ADMIN_THEME_PALETTE setting);
# changing AdminColors.PRIMARY is usually enough. Note the `--q-*` brand group
# must stay under `body` with `!important` — NiceGUI sets it as an inline body
# style, so a `:root` declaration is inert (that was the pre-#365 state).
